...
<div class="container-fluid">
	...
	<script>
			...
			function buscarUsuarios(palabra){
				...
				$.ajax({
					url: "<?php echo base_url(); ?>/usuarios/buscarUsuariosNoActivos",
					method: "POST",
					...
					datos.forEach(function(dato, index) {
						...
						if( dato.role < 3 ){
							document.getElementById("tablaUsuarios").insertRow(-1).innerHTML = '<td id="'+index+'1"></td><td id="'+index+'2"></td><td id="'+index+'3"></td><td id="'+index+'4"></td><td id="'+index+'5"></td><td id="'+index+'6"></td><td id="'+index+'7"></td></td><td id="'+index+'8"></td><td><a href="'+window.location.origin+'/usuarios/editar/'+dato.id+'" class="btn btn-dark"><i class="fas fa-pencil-alt"></i></a></td><td><a href="'+window.location.origin+'/usuarios/activar/'+dato.id+'" class="btn btn-dark"><i class="fas fa-arrow-up"></i></a></td>';
						} else {
